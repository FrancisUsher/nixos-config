...
- [x] hosts/red-sun-whorl/configuration.nix scaffolded (unbootable until
      hardware config is added - see below)
...
- [x] Headless captive-portal wifi login (modules/captive-portal.nix,
      services.captivePortalAccept, wired into red-sun-whorl)
- [x] hosts/red-sun-whorl/hardware-configuration.nix preview - generated via
...
- [x] Pull in NixOS/nixos-hardware's lenovo-thinkpad-x1-nano-gen1 module
      (flake input + import in hosts/red-sun-whorl/configuration.nix) - gives
...
- [ ] Laptop hardware-configuration.nix's fileSystems/swapDevices/
      ...
      per BOOTSTRAP.md's red-sun-whorl section) - don't trust these values to boot
...
- [x] Home Manager config shared between bubu-brain and laptop - flake.nix's
      mkHost takes a per-host username (bubu-brain: soong, red-sun-whorl:
      silk) and wires home-manager.users.${username} = import ./home.nix,
      with home.nix reading that username
